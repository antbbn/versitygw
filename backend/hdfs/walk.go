...
type GetObjFunc func(path string, info fs.FileInfo) (s3response.Object, error)
...
// objects responses
func (h *HDFS) Walk(ctx context.Context, bucket string, prefix, delimiter, marker string, max int32, getObj GetObjFunc, skipdirs []string) (WalkResults, error) {
	cpmap := cpMap{}
	...
	err := h.client.Walk(root, func(path string, info fs.FileInfo, err error) error {
		...
		if info.IsDir() {
			...
			if delimiter != "" &&
				...
				if delimiter == "" {
					dirobj, err := getObj(path+"/", info)
					if err == backend.ErrSkipObj {
						return skipflag
					}
					if err != nil {
